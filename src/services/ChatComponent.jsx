...
const ChatComponent = ({ receiverId, otherUserName }) => {
  ...
  const [messages, setMessages] = useState([]);
  const [newMessage, setNewMessage] = useState('');
  const [typingUser, setTypingUser] = useState(null); // ✅ show typing username
  const [typingTimeout, setTypingTimeout] = useState(null);
  ...
  useEffect(() => {
    ...
    const handleUserTypingStart = (data) => {
      ...
      const username =
        data?.username || data?.sender?.username || otherUserName || messagereceiverName;
    ...
    console.log("sadasdasd", typingUser)
    ...
    websocket.socket?.on('new_personal_message', handleNewPersonalMessage);
    websocket.socket?.on('typing_start', handleUserTypingStart);
    websocket.socket?.on('typing_stop', handleUserTypingStop);

    return () => {
      websocket.leaveConversation(receiverId);
      websocket.socket?.off('new_personal_message', handleNewPersonalMessage);
      websocket.socket?.off('typing_start', handleUserTypingStart);
      websocket.socket?.off('typing_stop', handleUserTypingStop);
  ...
  }, [receiverId, myUserId, websocket, fetchConversation, otherUserName, messagereceiverName]);
