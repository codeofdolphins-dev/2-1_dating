...
const SubscriptionCard = ({ 
  ...
  const handleSubscription = async () => {
    try {
      const res = await httpService("/payments/create-order", "POST", { planId });
      console.log("Order created:", res?.data);

      // ✅ Navigate & send API response as state
      navigate(`/subscription-moreInfo/${planId}`, {
        state: { orderData: res?.data }
      });
    } catch (err) {
      console.error("Error creating order:", err?.response?.data?.message || err.message);
