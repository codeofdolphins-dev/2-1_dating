...
const SecondRegistrationForm = () => {
    ...
    // ✅ Allow appending multiple images instead of overwriting
    const handleFileChange = (e) => {
        const files = Array.from(e.target.files);

        if (files.length > 0) {
            const newFiles = files.slice(0, 4 - profileImages.length); // only take what's needed
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            // ... your DOB + validation checks stay the same
            ...
            if (profileImages.length > 0) {
                ...
                profileImages.forEach((file) => {
                    imageForm.append("files", file); // multiple files key = "files"
                });
                ...
                // Extract array of URLs from API response
                uploadedImageUrls = uploadRes?.data?.results?.map((f) => f?.files?.original?.url) || [];
                console.log("✅ Uploaded Image URLs:", uploadedImageUrls);
            }

            // ✅ Step 2: Prepare payload (send array of URLs instead of single)
            const payload =
                formData.gender === "couple"
                    ? {
                        ...
                        photos: uploadedImageUrls, // <-- array of photos
                    }
                    : {
                        gender: formData.gender,
                        sexuality: formData.sexuality,
            ...
            if (response?.message) {
                console.log("response_secondRegistration",response?.data)
        ...
        <div className="container py-5 text-white" style={{ maxWidth: "500px" }}>
            ...
            <div className="card text-white p-4 rounded-4" style={{ backgroundColor: "var(--color-border)" }}>
                <form onSubmit={handleSubmit}>
                    ...
                    <Form.Group className="mb-3">
                        <Form.Label>Main Profile Pictures *</Form.Label>
                        <div className="d-flex flex-wrap gap-2">
                            ...
                            {/* ✅ Only show upload box if less than 2 images */}
                            {preview.length < 4 && (
                                <label
                                    htmlFor="photo-upload"
                                ...
                                >
                                    ...
                                    <input
                                        ...
                                        id="photo-upload"
                                        onChange={handleFileChange}
                                        multiple={false} // select one at a time
                                        style={{ display: "none" }}
                                    />
                                </label>
                            )}
