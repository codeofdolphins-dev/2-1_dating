// import React, { useEffect, useState } from 'react';
// import "./videos.css";
// import PageWrapper from '../../components/PageWrapper';
// import FilterBar from '../../components/FilterBar/FilterBar';
// import VideoCard from '../../components/Video Card/VideoCard';
// import VideoModal from '../../components/Video Modal/VideoModal';
// import { useNavigate } from 'react-router-dom';
// import httpService from '../../helper/httpService';
// import OverlayLoader from '../../helper/OverlayLoader';
// import { useAuth } from '../../context/AuthContextAPI';

//   const filter2 = [
//     "Latest",
//     "Distance",
//     "Most Viewed",
//     "Couples",
//     "Female",
//     "Male",
//     "Businessess",
//     "Transgender",
//     "Friends",
//     "Past Live Streams"
//   ]

// const Videos = () => {
//   const navigate = useNavigate();
//   const {filterOption} = useAuth()

//   const navigationToAnotherPage = () => {
//     navigate("/current-user-profile");
//   };

//   const [show, setShow] = useState(false);
//   const [videos, setVideos] = useState([]);
//   const [load, setload] = useState(false)

//   const videoApiCall = async () => {
//     setload(true)
//     try {
//       const res = await httpService(
//         `/media-library/global/videos?adultContent=all&contentRating=all&visibility=alluserTypes=${filterOption.toLowerCase()}`
//       );
//       setVideos(res?.data?.videos || []);
//     } catch (err) {
//       console.log(err);
//     }
//     setload(false)
//   };

//   useEffect(() => {

//     videoApiCall();

//   }, [filterOption]);

//   // console.log("filterOption",filterOption.toLowerCase())

//   const handleOpenModal = () => setShow(true);

//   const handleModalClose = async () => {
//     setShow(false);
//     await videoApiCall(); // ✅ refresh list when modal closes
//   };



//   return (
//     <>
//       <PageWrapper>
//         <OverlayLoader show={load} />
//         <div
//           className="container-fluid pt-0 pb-5 d-flex flex-column align-items-start justify-content-start gap-2"
//           style={{ backgroundColor: "var(--color-background)", minHeight: "100vh" }}
//         >
//           <FilterBar
//             pageName={"Videos"}
//             navigationPageName1={"Add Videos"}
//             filterName2={"Filter"}
//             filter2={filter2}
//             navigationToAnotherPage={navigationToAnotherPage}
//             checkbox={false}
//             okButton={true}
//           />

//           <div className="client-page-background">
//             <div className="container-fluid">
//               <div className="row g-4 pt-4">
//                 {videos.map((card, index) => (
//                   <div
//                     className="col-12 col-sm-6 col-lg-6 col-xl-4 card-container p-0 m-0"
//                     key={index}
//                   >
//                     <VideoCard onClick={handleOpenModal} card={card} />
//                   </div>
//                 ))}

//                 <VideoModal show={show} handleClose={handleModalClose} />
//               </div>
//             </div>
//           </div>
//         </div>
//       </PageWrapper>
//     </>
//   );
// };

// export default Videos;




import React, { useEffect, useState, useRef, useCallback } from 'react';
import "./videos.css";
import PageWrapper from '../../components/PageWrapper';
import FilterBar from '../../components/FilterBar/FilterBar';
import VideoCard from '../../components/Video Card/VideoCard';
import VideoModal from '../../components/Video Modal/VideoModal';
import { useNavigate } from 'react-router-dom';
import httpService from '../../helper/httpService';
import OverlayLoader from '../../helper/OverlayLoader';
import { useAuth } from '../../context/AuthContextAPI';

const filter2 = [
  "Latest",
  "Distance",
  "Most Viewed",
  "Couples",
  "Female",
  "Male",
  "Businessess",
  "Transgender",
  "Friends",
  "Past Live Streams"
];

const Videos = () => {
  const navigate = useNavigate();
  const { filterOption } = useAuth();

  const navigationToAnotherPage = () => {
    navigate("/current-user-profile");
  };

  const [show, setShow] = useState(false);
  const [videos, setVideos] = useState([]);
  const [load, setLoad] = useState(false);
  const [page, setPage] = useState(1);
  const [hasMore, setHasMore] = useState(true);

  const isFetching = useRef(false);

  const videoApiCall = useCallback(
    async (pageNum = 1, reset = false) => {
      if (isFetching.current || (!hasMore && !reset)) return;

      isFetching.current = true;
      setLoad(true);

      try {
        const res = await httpService(
          `/media-library/global/videos?adultContent=all&contentRating=all&visibility=all&page=${pageNum}&limit=12`
        );

        const newVideos = res?.data?.videos || [];

        if (reset) {
          setVideos(newVideos);
        } else {
          setVideos((prev) => [...prev, ...newVideos]);
        }

        if (videos.length === 0) {
          setHasMore(false); // ✅ stop further calls
        }
      } catch (err) {
        console.error("Video API error:", err);
      } finally {
        setLoad(false);
        isFetching.current = false;
      }
    },
    [filterOption, hasMore]
  );

  // Reset when filter changes
  useEffect(() => {
    setPage(1);
    setHasMore(true);
    videoApiCall(1, true);
  }, [filterOption, videoApiCall]);

  // Infinite scroll
  useEffect(() => {
    const handleScroll = () => {
      if (
        hasMore &&
        !load &&
        window.innerHeight + document.documentElement.scrollTop + 100 >=
          document.documentElement.scrollHeight
      ) {
        setPage((prev) => prev + 1);
      }
    };

    window.addEventListener("scroll", handleScroll);
    return () => window.removeEventListener("scroll", handleScroll);
  }, [hasMore, load]);

  // Fetch when page changes
  useEffect(() => {
    if (page > 1 && hasMore) {
      videoApiCall(page);
    }
  }, [page, hasMore, videoApiCall]);

  const handleOpenModal = () => setShow(true);

  const handleModalClose = () => setShow(false);

  return (
    <PageWrapper>
      <OverlayLoader show={load && page === 1} />
      <div
        className="container-fluid pt-0 pb-5 d-flex flex-column align-items-start justify-content-start gap-2"
        style={{ backgroundColor: "var(--color-background)", minHeight: "100vh" }}
      >
        <FilterBar
          pageName={"Videos"}
          navigationPageName1={"Add Videos"}
          filterName2={"Filter"}
          filter2={filter2}
          navigationToAnotherPage={navigationToAnotherPage}
          checkbox={false}
          okButton={true}
        />

        <div className="client-page-background">
          <div className="container-fluid">
            <div className="row g-4 pt-4">
              {videos.map((card, index) => (
                <div
                  className="col-12 col-sm-6 col-lg-6 col-xl-4 card-container p-0 m-0"
                  key={index}
                >
                  <VideoCard onClick={handleOpenModal} card={card} />
                </div>
              ))}

              <VideoModal show={show} handleClose={handleModalClose} />
            </div>

            {/* Loader when fetching next page */}
            {load && page > 1 && (
              <div className="text-center py-3 text-white">Loading more...</div>
            )}

            {/* No more data */}
            {!hasMore && !load && (
              <div className="text-center py-3 text-muted">
                No more videos
              </div>
            )}
          </div>
        </div>
      </div>
    </PageWrapper>
  );
};

export default Videos;
