...
const ProfileLocation = () => {
  ...
  const swapLocationHandler = () => {
    setPrimaryLocation((prevPrimary) => {
      const temp = secondaryLocation;
      setSecondaryLocation(prevPrimary);
      return temp;
    });
  };

  useEffect(() => {
    httpService("/auth/me", "GET").then((res) => {
      console.log("response", res?.data?.profile?.address?.fullAddress);
      setPrimaryLocation(res?.data?.profile?.address?.fullAddress || "");
      setSecondaryLocation(res?.data?.profile?.address?.fullAddress || "");
    });
  }, []);
        ...
        <div className="container-fluid mt-5 pt-5 px-3 d-flex flex-column align-items-start justify-content-start gap-2 text-white">
          <div className="w-100 d-flex flex-column gap-3" style={{ color: "#B0C3CC" }}>
            ...
            <div className="d-flex justify-content-between align-items-center flex-wrap gap-4">
              ...
              <div
                className=""
                style={{ borderBottom: "2px solid #343A40", width: "274px" }}
              >
                <label
                  className="form-label mb-0"
                  htmlFor=""
                  style={{ fontSize: "13px", color: "#B0C3CC" }}
                >
              ...
              >
                ...
                <div className="d-flex align-items-center mt-2">
                  <img
                    ...
                    width={"13px"}
                    height={"13px"}
                    onClick={removeSecondaryLocationHandler}
